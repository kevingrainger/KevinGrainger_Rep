...
void SOR() {
    ...
    ofstream csvFile("global_max_delta_Neumann.csv");
...
int main() {
    ...
    SOR();                                  // SOR to solve for phi, and call Neumann boundary function each run

    cout << "∂ϕ/∂y(2/5, 1/2) = " << derivative_value() << endl;     //Printing derivative value

    // Printing the grid values in CSV format as I used excel to graph these funcitions
    ofstream csvFile("phi_Neumann_values.csv");
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {

            csvFile <<phi[i][j]<<",";                      //Storing the phi values
        }
        csvFile<<endl;                                     //Excel needs new lines for each seciton of the grid
    }
    csvFile.close();

    cout << "phi values saved in 'phi_Neumann_values.csv' and global_max_delta saved in 'global_max_delta_Neumann.csv'" << endl;
    return 0;
}