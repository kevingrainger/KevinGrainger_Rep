...
TOTAL_STEPS = 100
PLOT_EVERY = 5
BETA = 0.15  # Interaction strength parameter (1/kT)
...
class ReefSimulation:
    ...
    def update_temperature(self):
        ...
        # Update Degree Heating Days (DHD)
    ...
    def calculate_potts_energy(self, x, y, state):
       #Calculate Potts model energy for a given state at position
        ...
        for neighbor_state in neighbors:
            if neighbor_state == state and neighbor_state != 0:  # Don't count empty sites
                energy -= 1.0  # coupling
        
        # Environmental energy
        if state == HEALTHY_CORAL:
            # Penalty for high temperature and accumulated heat stress
            if temp > self.bleaching_thresh:
                stress_factor = (temp - self.bleaching_thresh) * (1 + 0.05 * self.dhd_grid[x,y])
                energy += 2.0 * stress_factor
            else:
                energy -= 0.5  # Bonus for good conditions
                
        elif state == BLEACHED_CORAL:
            # Mild penalty for being bleached
            energy += 0.5
            # Recovery bonus with cool temperatures and healthy neighbors
    ...
    def metropolis_step(self):
        ...
        # Perform GRID_SIZE^2 attempted moves per step
        for _ in range(GRID_SIZE * GRID_SIZE):
            ...
            # Choose new state based on allowed coral transitions
    ...
    def get_allowed_transitions(self, current_state, x, y):
        ...
        if current_state == HEALTHY_CORAL:
            ...
            # Higher bleaching probability with temperature and DHD
            if temp > self.bleaching_thresh:
                transitions.append(BLEACHED_CORAL)
            return transitions
            
        elif current_state == BLEACHED_CORAL:
            # Bleached coral can recover, die or stay bleached
            transitions = [BLEACHED_CORAL]
            # Recovery possible with cool temperatures
            ...
            # Coral Creation possible with sufficient healthy neighbors
            transitions = [0]
            ...
            for y in range(GRID_SIZE):
                ...
                # Recovery and mortality
                    ...
                    if temp < self.bleaching_thresh - 1.0:
                        # Recovery enhanced by healthy neighbors
    ...
    def plot_populations(self, ax):
        ...
        # Plot target bleaching data if available
...
def run_animation(excel_file="Coral temps.xlsx", bleaching_thresh=30.0, dhd_decay=0.9):
    ...
    # Run animation for the length of temperature data
...
if __name__ == "__main__":
    try:
        ...
        # Run final simulation with optimized parameters (non-animated)
        print("\n" + "="*60)
